<?php
$page_title = "Women's Collection - LuxeMode";
...
$total = $conn->query("SELECT COUNT(*) FROM products WHERE category='womens' $sale_filter")->fetch_row()[0];
$pages = ceil($total / $per);
$products = $conn->query("SELECT * FROM products WHERE category='womens' $sale_filter ORDER BY $order LIMIT $per OFFSET $offset");
?>
...
<div class="container" style="padding:30px 24px;">
    <div class="filter-wrap">
        <form method="GET" style="display:flex;gap:12px;flex-wrap:wrap;align-items:flex-end;width:100%;">
            ...
            <a href="womens.php" class="btn btn-outline btn-sm">Reset</a>
        </form>
    </div>
